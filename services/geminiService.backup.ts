...
export const optimizeScriptWithGemini = async (
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    const prompt = `
    You are an expert podcast script editor.
    Target Audience: Chinese listeners of a tech/business podcast.
    
    Task:
    1. **Rewrite** the "translatedChinese" to be **highly colloquial, natural, and energetic**. Remove stiff "translation-ese". 
       - e.g. "I cannot believe" -> "我简直不敢相信"
       - e.g. "Well, but" -> "哎，不过"
    2. **Insert Audio Tags** at the START or within the text to guide the TTS engine.
